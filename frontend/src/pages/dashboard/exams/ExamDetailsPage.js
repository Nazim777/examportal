...
function ExamDetailsPage() {
    ...
    async function handleExam(id){
        try{
           const {data} = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/exams`,{withCredentials:true});
           const val = data.exams.find(exam=>exam._id===id);
           const res = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/exam/${id}`,{exampass:val.exampass},{withCredentials:true});
